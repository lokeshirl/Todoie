...
const Login = () => {
  // State to manage input data {email, password}
  const [loginData, setLoginData] = useState({ email: "", password: "" });
  // const [error, setError] = useState(null);
  // Accessing the login function from Auth Context
  const { error, login } = useAuth();

  // function to update state when input data changes
  const handleLoginDataChange = (name) => (e) => {
        ...
        <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form className="space-y-4 md:space-y-6" onSubmit={handleUserLogin}>
              ...
              {error && (
                <div className="border text-[#e7195a] rounded mx-0 my-5 p-2.5 border-solid border-[#e7195a] ">
                  {error}
                </div>
              )}
            </form>
